#!/usr/bin/env python3
"""
Test the updated stroke connection algorithm with crossbar inclusion (no numpy).
"""
...
def test_smart_crossbar_connection():
    ...
    crossbar_points = crossbar['points'][:]  # Copy the list
    ...
    dist_to_start = distance(right_end, crossbar_start)
    dist_to_end = distance(right_end, crossbar_end)
    ...
    right_diagonal_end = connected_points[5]  # Last point of right diagonal
    crossbar_connection = connected_points[6]  # Connection point to crossbar
    jump_distance = distance(right_diagonal_end, crossbar_connection)
    print(f"- Jump distance: {jump_distance:.1f} pixels")
...
if __name__ == "__main__":
    ...
    print("- No more missing crossbar")
    print("- Minimal unwanted jump line")
